...
#how to run: bash server_client_classification.sh 1 8000 
...
do
    ...
    do        
        ...
        sleep 4 #how can we get rid of that?

        # Start clients
        for party_id in $(seq 1 $parties)
        do
            python -u client_classification.py --n_features 30 --k $k --party_id $party_id --n_samples 400 --base_port $port --max_parties $parties
            sleep 0.1 #how can we get rid of that?
        done
