...
def get_labels(n_samples, num_clients, n_features):
    """
    ...
    training_data, y = make_classification(n_samples=temp2, n_features=n_features, random_state=42, shuffle=False)
    return y
...
def train_kernel(args): 
    ...
    #else: 
    y = get_labels(args.n_samples, args.num_clients, n_features)

    clf = svm.SVC(kernel = 'precomputed', probability=True)
    clf.fit(sklearn.metrics.pairwise.polynomial_kernel(full, Y=None, degree=3, gamma=None, coef0=1), y)
    ...
    Kernel_lib.metrics_micro(clf, full, y, cv=5)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_samples", type=int, default=400, help="dataset size for orty")
